...
def set_params(feat_des='sift',dense=False,level=0,k=128,nfeatures=300,
               hessianThreshold=600,step=10,pca_perc=0.8,lda_ncomponents=7):
    params = {
...
def run(train_pyramid_descriptors, D, test_pyramid_descriptors, feat_des_options):
    ...
    knnpca = LogisticRegression(solver = 'saga')
    ...
    lda = LinearDiscriminantAnalysis(n_components=feat_des_options['lda_ncomponents'])
    VWlda = lda.fit_transform(visual_words_pyramid,train_labels)
    knnlda = LogisticRegression(solver = 'saga')
    knnlda.fit(VWlda, train_labels)
    vwtestlda = lda.transform(visual_words_test)
    lda_test_accuracy = 100*knnlda.score(vwtestlda, test_labels)
    # print("LDA Test accuracy: %0.2f" % (lda_test_accuracy))
    scores_lda = cross_validate(knnlda, VWlda, train_labels,
                                scoring = ['accuracy', 'precision_macro', 'recall_macro','f1_macro'],
                                cv=8, return_estimator=True)
    cross_val_accuracy_lda = scores_lda['test_accuracy'].mean()
    cross_val_precision_lda = scores_lda['test_precision_macro'].mean()
    cross_val_recall_lda = scores_lda['test_recall_macro'].mean()
    cross_val_f1_lda = scores_lda['test_f1_macro'].mean()
    ...
    return [cross_val_accuracy, cross_val_precision, cross_val_recall, cross_val_f1, test_precision,
           ...
           cross_val_precision_pca, cross_val_recall_pca, cross_val_f1_pca, cross_val_accuracy_lda,
           cross_val_precision_lda, cross_val_recall_lda, cross_val_f1_lda]
